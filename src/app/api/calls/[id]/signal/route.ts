...
export async function POST(request: Request, ctx: Ctx) {
  ...
  if (!user) return jsonError("نشست فعال نیست.", 401);
  const { id } = await ctx.params;
  const body = (await request.json().catch(() => null)) as { type?: string; body?: string; nonce?: string } | null;
  const type = body?.type as CallSignal["type"] | undefined;
  if (!type) return jsonError("سیگنال نامعتبر است.");
  const result = await postCallSignal(user.id, id, { type, body: body?.body, nonce: body?.nonce });
  if (!result.ok) return jsonError(result.error, result.status);
