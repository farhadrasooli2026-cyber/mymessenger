...
export default function JoinGroupPage() {
  ...
  const [info, setInfo] = useState<{ name: string; description: string; color: string; memberCount: number; rules: string; joinMode: string } | null>(null);
      ...
      <div className="w-full max-w-md rounded-3xl border border-white/10 bg-[#102824] p-6 text-center">
        ...
        {info && (
          <>
            ...
            <p className="mt-1 text-xs text-emerald-100/60">{info.memberCount} عضو</p>
            <p className="mt-3 text-sm leading-7">{info.description}</p>
            {info.rules && <p className="mt-3 whitespace-pre-wrap text-right text-xs leading-6 text-emerald-100/70">{info.rules}</p>}
            ...
            <Button type="button" className="mt-5 h-11 w-full bg-amber-300 text-[#102824]" onClick={() => void join()}>
              {info.joinMode === "request" ? "ارسال درخواست عضویت" : "پیوستن به گروه"}
            </Button>
          </>
        )}
      </div>
