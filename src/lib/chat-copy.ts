...
export const SEED_PEERS = [
  ...
  },
  {
    ...
    messages: [
      ...
      "گروه را از لیست گفتگوها بساز. کانال هنوز روی نقشه است.",
    ],
  },
  {
    peerKey: "noor",
...
export function nixoLocalReply(text: string): string {
  ...
  if (/گروه|کانال|فروش|کیف|پرداخت|ai|ربات|بلاک|گزارش|عکس|ویدیو|فایل|مدیا/i.test(text)) {
    return "رسانهٔ خصوصی E2EE است. گروه، جامعه و کانال زنده‌اند. پست کانال برای دنبال‌کننده‌ها روی سرور است، نه داخل کلید چت خصوصی.";
  }
