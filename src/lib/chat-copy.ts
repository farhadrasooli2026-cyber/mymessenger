...
export function nixoLocalReply(text: string): string {
  if (/صوت|ویس|voice|امن|هک|hack|security|رمز|دستگاه|نشست|otp|passkey|دومرحله/i.test(text)) {
    return "امنیت نیکسو تضمین مطلق نیست؛ لایه‌لایه است: احراز هویت سخت، نشست قابل ابطال، رمز دومرحله‌ای، و رمزنگاری سرتاسری روی دستگاه تو. متن پیام روی سرور نمی‌ماند. هر ادعای «هرگز هک نمی‌شویم» را باور نکن. مسیر تنظیمات → امنیت را ببین.";
  }
  if (/گروه|کانال|فروش|کیف|پرداخت|ai|ربات|بلاک|گزارش|عکس|ویدیو|فایل|مدیا|استوری|وضعیت|جستجو|ذخیره|حریم|پرایوسی/i.test(text)) {
    return "حریم نیکسو روی سرور است نه فقط روی صفحه. متن چت خصوصی روی دستگاه می‌ماند. حساب به‌خاطر ورود نکردن حذف نمی‌شود. تنظیمات ← حساب و حریم خصوصی را ببین.";
  }
